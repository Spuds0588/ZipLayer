...
**jsDelivr (npm)** — the package metadata (`exports`, `files`, `jsdelivr`, `unpkg`) is already configured; once published to npm it's available as:

```html
<script type="module">
  import { ZipLayer } from "https://cdn.jsdelivr.net/npm/ziplayer@0.1.0/src/ziplayer.js";
</script>
```

> Publishing to npm (`npm publish`) is a one-time manual step that requires your npm account — nothing else needs to change.

---

## Live Demo

👉 **Open the home page → https://spuds0588.github.io/ZipLayer/** (or just open `index.html`) and click **⬇ Download Example Zip** to launch the wizard with a simulated mortgage loan file (`assets/JohnSmith-LoanFile.zip`) — W-2, pay stub, sales contract, and driver's license for a fictitious borrower.

> The example archive is generated by `scripts/make-example.mjs` (run `node scripts/make-example.mjs` to regenerate) and served as a static asset.
...
node scripts/make-example.mjs   # regenerate the example zip asset
```
